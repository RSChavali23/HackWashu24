...
if __name__ == "__main__":
    input_image = "./output/test1_final.png"  # Replace with your image path

    # Extract the base name without extension
    base_name = os.path.splitext(os.path.basename(input_image))[0]

    # Define the output directory
    output_dir = "./3Doutput/"  # Desired output directory

    # Ensure the output directory exists
    os.makedirs(output_dir, exist_ok=True)

    # Construct the output file path with the same base name and .obj extension
    output_obj = os.path.join(output_dir, f"{base_name}.obj")

    # Define other parameters
    scale_factors = (1.0, 1.0, 1.0)  # Adjust as needed (x, y, z)
    alpha_thresh = 100               # Adjust based on image transparency
    downscale = 2                    # Adjust to reduce mesh resolution
